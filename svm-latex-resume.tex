...
\usepackage{hyperref}
$if(colorlinks)$
\PassOptionsToPackage{usenames,svgnames}{color} % color is loaded by hyperref
$endif$
...
$if(email)$$email$ \hspace{25 mm}$endif$
$if(phone)$$phone$ \hspace{25 mm}$endif$
$if(location)$$location$ \hspace{25 mm}$endif$
}
\vspace{-1em}
\centering{\normalsize
$if(linkedin)$$if(fontawesome)$\faLinkedin \hspace{1 mm}$else$\textbf{LinkedIn:}$endif$ \href{https://www.linkedin.com/in/$linkedin$}{\tt $linkedin$} \hspace{25 mm}$endif$
$if(github)$$if(fontawesome)$\faGithub \hspace{1 mm}$else$\textbf{Github:}$endif$ \href{http://github.com/$github$}{\tt $github$} \hspace{1 mm}$endif$
}
\vspace{2em}
\flushleft{\large
